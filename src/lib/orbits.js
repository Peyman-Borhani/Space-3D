...
export {createOrbit,    getOrbitPosition,   JulianDateToTrueAnomaly}
...
function  createOrbit(orbitParams, color, n_mesh_points)
{
    ...
    for (let i = 0; i <= n_mesh_points; i++) {
        ...
        points.push(new Vector3(xCamera, zCamera, -yCamera));
    }
...
function  getOrbitPosition(a, e, trueAnomaly, matrix)
{
    ...
    return new Vector3(xCamera, zCamera, -yCamera);
}
...
{   //JD and epoch need to use the same reference system (either MJD or JD) 
    const mu = 0.0002959122082855911;//correct number: 0.0002959122082855911025;
...
const  computeTrueAnomaly  =(E, e)=> 2*Math.atan(Math.sqrt((1+e) / (1-e)) * Math.tan(E/2));
...
function solveKepler(e, M)
{
    ...
    if (M < 0) {
        while (true) {
            ...
            if (MDiff > 0) {minBound =E;   break}
            else {maxBound =E;     E *=EMult}
        }
    }
    else {
        while (true) {
            ...
            MDiff = M - MTest;
            if (Math.abs(MDiff) < espLim) {return  E}
            if (MDiff > 0) {minBound =E;    E *=EMult}
            else {maxBound =E;  break}
        }
    }
